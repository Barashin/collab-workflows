...
- **出力**: `ligand/output/selected_compounds/` ディレクトリ（選択されたSDFファイル）
- **説明**: 選択されたリガンドを個別のSDFファイルとして保存
- **注意**: デフォルトではテスト用にランダムに29個のファイルを選択

### Node 4: リガンド準備
- **ファイル**: `ligand/node_04_prepare_ligands.py`
- **入力**: `ligand/output/selected_compounds/` ディレクトリ内のSDFファイル群
- **出力**: `ligand/output/selected_compounds/` ディレクトリ（準備済みSDFファイル）
- **説明**: リガンドに水素を追加、電荷を割り当て、3D構造を生成し、エネルギー最小化を実行

### Node 5: PDBファイルダウンロード
- **ファイル**: `protein/node_05_download_pdb.py`
- **入力**: `PDB_ID` (環境変数またはデフォルト値 "5Y7J")
- **出力**: `protein/output/{pdb_id}.pdb` (例: `5Y7J.pdb`)
- **説明**: RCSB PDBからタンパク質構造ファイルをダウンロード

### Node 6: タンパク質入力確認
- **ファイル**: `protein/node_06_protein_input.py`
- **入力**: `PDB_ID` (環境変数またはデフォルト値)
- **出力**: `protein/output/{pdb_id}.pdb` (確認のみ)
- **説明**: PDBファイルが存在することを確認

### Node 7: リガンドローディング
- **ファイル**: `preparation/node_07_ligand_loading.py`
- **入力**: `protein/output/{pdb_id}.pdb`
- **出力**: `preparation/output/ligand_list.txt`
- **説明**: PDBファイルからリガンド情報を抽出

### Node 8: チェーン抽出
- **ファイル**: `preparation/node_08_extract_chains.py`
- **入力**: `protein/output/{pdb_id}.pdb`
- **出力**: `preparation/output/{pdb_id}_chain.pdb`, `preparation/output/real_ligand.sdf`
- **説明**: PDBファイルから指定されたチェーンとリファレンスリガンドを抽出

### Node 9: 実リガンド追加
- **ファイル**: `ligand/node_09_real_ligand_addition.py`
- **入力**: `ligand/output/selected_compounds/` ディレクトリ, `preparation/output/real_ligand.sdf`
- **出力**: `ligand/output/selected_compounds/` ディレクトリ（実リガンドを含む）
- **説明**: 準備済みリガンドとPDBから抽出した実リガンドを結合
...
- **入力**: `preparation/output/ligand_list.txt`, `preparation/output/{pdb_id}_chain.pdb`
- **出力**: `preparation/output/config.txt`
- **説明**: リガンドの中心座標を計算し、ドッキング設定ファイルを生成

### Node 12: タンパク質クリーンアップ
...
### Node 13: SMINA - インシリコスクリーニング
- **ファイル**: `docking/node_13_smina_screening.py`
- **入力**: 
  - `preparation/output/config.txt`
  - `preparation/output/{pdb_id}_clean.pdb`
  - `ligand/output/selected_compounds/` (個別のSDFファイル)
- **出力**: `docking/output/` ディレクトリ（ドッキング結果ファイル群）
- **説明**: SMINAを使用して分子ドッキングを実行

### Node 14: レポート生成
- **ファイル**: `report/node_14_reporting.py`
...
```
1. Node 1 (リガンドダウンロード) ─┐
2. Node 5 (PDBダウンロード)      ─┤ 並列実行可能
                                   │
3. Node 2 (リガンド展開) ← Node 1
4. Node 6 (タンパク質入力確認) ← Node 5
5. Node 3 (リガンド選択) ← Node 2
6. Node 4 (リガンド準備) ← Node 3
7. Node 7 (リガンドローディング) ← Node 5
8. Node 8 (チェーン抽出) ← Node 5
                                   │
9. Node 9 (実リガンド追加) ← Node 4, 8
10. Node 10 (リガンドビュー) ← Node 9
11. Node 11 (リガンド中心識別) ← Node 7, 8
12. Node 12 (タンパク質クリーンアップ) ← Node 8
                                  │
13. Node 13 (SMINAスクリーニング) ← Node 11, 12
14. Node 14 (レポート生成) ← Node 13
```
...
1. **Node 3 (リガンド選択)**: デフォルトではランダムに29個のリガンドを選択します。選択数を変更する場合は、`ligand/node_03_ligand_selection.py` の `DEFAULT_NUM_LIGANDS` を変更してください。

2. **Node 13 (SMINAスクリーニング)**: `ligand/output/selected_compounds/` ディレクトリ内の個別のSDFファイルを処理します。
